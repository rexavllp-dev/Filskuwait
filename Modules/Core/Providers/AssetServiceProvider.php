...
use Modules\Core\Foundation\Asset\Manager\FilsKuwaitAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\FilsKuwaitAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, FilsKuwaitAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new FilsKuwaitAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("filskuwait.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("filskuwait.modules.{$module->getAlias()}.assets");
